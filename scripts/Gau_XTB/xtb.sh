...
if [ -n "$XTB_PATH" ] ; then
	if [ -x "$XTB_PATH" ] ; then
		XTB_BIN="$XTB_PATH"
	elif [ -x "$XTB_PATH/bin/xtb" ] ; then
		XTB_BIN="$XTB_PATH/bin/xtb"
	fi
fi
if ! command -v "$XTB_BIN" >/dev/null 2>&1 && [ ! -x "$XTB_BIN" ] ; then
	echo "Error: xtb executable not found (XTB_PATH=$XTB_PATH)"
	exit 1
fi
uhf=`echo "$spin-1" | bc` #nalpha-nbeta
if [ $derivs == "2" ] ; then
	echo "Running: $XTB_BIN mol.xyz --chrg $charge --uhf $uhf --hess --grad --gfn 2 > xtbout"
	"$XTB_BIN" mol.xyz --chrg $charge --uhf $uhf --hess --grad --gfn 2 > xtbout
elif [ $derivs == "1" ] ; then
	echo "Running: $XTB_BIN mol.xyz --chrg $charge --uhf $uhf --grad --gfn 2 > xtbout"
	"$XTB_BIN" mol.xyz --chrg $charge --uhf $uhf --grad --gfn 2 > xtbout
fi
