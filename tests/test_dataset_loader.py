...
def test_dataset_loader_keeps_metadata_only_without_bond_aliases(tmp_path: Path) -> None:
    ...
    assert records[0].raw.get("map_status") == "OK"
    ...
    assert "forming_bonds_index_base" not in records[0].raw
    assert "index_base" not in records[0].raw
    assert records[0].raw.get("reaction_profile") == "[4+3]_default"


def test_dataset_loader_supports_prefixed_filter_ids(tmp_path: Path) -> None:
    dataset_file = tmp_path / "dataset.csv"
    _ = dataset_file.write_text(
