...
| Benchmark Runner | `benchmark/run_benchmark.sh --rx-id 1` |
...
4. **evaluate.py report** — the auto-generated `summary.md` showed all protocols as ❌ because it was run from a previous partial run. The provenance.json files confirm all 4 protocols completed successfully. Re-running `benchmark/evaluate.py` would regenerate the report correctly.
