...
> Note: the legacy `forward_scan` alias was removed in v2.1.1; references below are historical notes or migration context.
...
- **Config Example**:
  ...
  reaction_profiles:
    "[4+3]_default":
      ...
      s2_strategy: forward_scan  # historical config; removed in v2.1.1
      scan:
...
| `tests/test_forward_scan_wiring.py` (removed in v2.1.1) | Historical forward-scan wiring tests |
