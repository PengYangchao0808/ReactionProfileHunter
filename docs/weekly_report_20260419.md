...
| 文件 | 行数 | 功能 |
|------|------|------|
| `benchmark/run_benchmark.sh` | 468 | Bash 编排器：按反应×协议组合生成临时配置，调用 `python -m rph_core --skip-steps s2,s3,s4`，验证 provenance.json + product_min.xyz |
| `benchmark/evaluate.py` | 672 | Python 后评估：解析 provenance.json / conformer_energies.json，计算 funnel 统计、能量对比、交叉协议排名，输出 markdown summary.md + JSON summary.json |
| `benchmark/test_s1_protocols.sh` | 316 | 单轮测试脚本：循环切换协议运行 S1，验证 provenance 字段 |
...
**自动化实施**：
- 修改 `benchmark/run_benchmark.sh` 增加 `--theory` 参数，接受覆写配置
- 创建 `benchmark/theory_configs/` 目录存放各实验的 YAML override
...
| Benchmark 脚本 | `benchmark/run_benchmark.sh` | 运行编排器 |
| 评估脚本 | `benchmark/evaluate.py` | 后评估计算 |
