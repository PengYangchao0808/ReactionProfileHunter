...
class LogParser:
    ...
    def extract_last_converged_coords(
        ...
        if engine_type == 'gaussian':
            ...
            return LogParser._parse_orca_out(content)
        else:
    ...
    def _parse_gaussian_log(content: str) -> Tuple[Optional[np.ndarray], Optional[List[str]], Optional[str]]:
        ...
        ```
        """
        # 搜索所有 Standard Orientation 块
        standard_pattern = r'Standard orientation:\s*\n-{20,}\s*\n\s*Center.*?Coordinates \(Angstroms\).*?-{20,}\s*\n((?:\s*\d+\s+\d+\s+\d+\s+[\-\d\.]+\s+[\-\d\.]+\s+[\-\d\.]+\s*\n)+)'

        all_matches = list(re.finditer(standard_pattern, content, re.IGNORECASE | re.DOTALL))

        if not all_matches:
            # 回退：尝试 Input Orientation
            input_pattern = r'Input orientation:\s*\n-{20,}\s*\n\s*Center.*?Coordinates \(Angstroms\).*?-{20,}\s*\n((?:\s*\d+\s+\d+\s+\d+\s+[\-\d\.]+\s+[\-\d\.]+\s+[\-\d\.]+\s*\n)+)'
            input_matches = list(re.finditer(input_pattern, content, re.IGNORECASE | re.DOTALL))

            if not input_matches:
                return None, None, "未找到 Standard Orientation 或 Input Orientation 块"

            # 使用最后一个 Input Orientation
            coords_text = input_matches[-1].group(1)
            logger.info("使用最后一个 Input Orientation 块")
        else:
            # 使用最后一个 Standard Orientation
            coords_text = all_matches[-1].group(1)
    ...
    def _parse_orca_out(content: str) -> Tuple[Optional[np.ndarray], Optional[List[str]], Optional[str]]:
        ...
        """
        # 方案1: CARTESIAN COORDINATES (ANGSTROEM)
        angstrom_pattern = r'CARTESIAN COORDINATES\s*\(ANGSTROEM\).*?\n((?:[A-Za-z]+\s+[\-]?\d+\.\d+\s+[\-]?\d+\.\d+\s+[\-]?\d+\.\d+\s+[\-]?\d+\.\d+\n)+)'

        angstrom_matches = list(re.finditer(angstrom_pattern, content, re.IGNORECASE | re.DOTALL))

        if angstrom_matches:
            coords, symbols = LogParser._parse_coord_block(angstrom_matches[-1].group(1))
            if coords is not None:
                logger.info("从最后一个 CARTESIAN COORDINATES (ANGSTROEM) 提取坐标")
                return coords, symbols, None

        # 方案2: CARTESIAN COORDINATES (A.U.) + 转换
        au_pattern = r'CARTESIAN COORDINATES\s*\(A\.U\.\).*?\n((?:[A-Za-z]+\s+[\-]?\d+\.\d+\s+[\-]?\d+\.\d+\s+[\-]?\d+\.\d+\s+[\-]?\d+\.\d+\n)+)'

        au_matches = list(re.finditer(au_pattern, content, re.IGNORECASE | re.DOTALL))

        if au_matches:
            bohr_to_angstrom = 0.529177210903
            coords, symbols = LogParser._parse_coord_block(au_matches[-1].group(1))
            if coords is not None:
