...
class TSOptimizer(LoggerMixin):
    ...
    def run_with_qctaskrunner(
        ...
        # V6.2: Run post-QC enrichment (DIAS/ASM)
    ...
    def _build_sp_matrix(
        ...
        if forming_bonds:
            reactant_coords, reactant_symbols = read_xyz(reactant)
            ts_coords, ts_symbols = read_xyz(ts_final_xyz)
            from .intramolecular_fragmenter import IntramolecularFragmenter

            fragmenter = IntramolecularFragmenter()
            fragmenter_config = self.config.get("step3", {}).get("enrichment", {}).get("fragmenter", {})
            frag_result = fragmenter.fragment(
                reactant_coords=reactant_coords,
                reactant_symbols=reactant_symbols,
                ts_coords=ts_coords,
                ts_symbols=ts_symbols,
                forming_bonds=forming_bonds,
                config=fragmenter_config
            )

            if frag_result.status == "ok":
                frag_a_indices = frag_result.fragA_indices
                frag_b_indices = frag_result.fragB_indices
            else:
                (bond1, bond2) = forming_bonds
                split_idx = bond1[1]
                frag_a_indices = list(range(0, split_idx))
                frag_b_indices = list(range(split_idx, len(ts_symbols)))
